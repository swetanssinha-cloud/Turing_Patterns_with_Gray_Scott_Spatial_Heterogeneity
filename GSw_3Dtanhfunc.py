...
for i in range(N):
  for j in range(N):
    ...
    z1 = (4 * i + j - 3*N//2) / transition
    z2 = -(4 * i + j - 3 * N) / transition
    z3 = (-4 * i + j + N//2) / transition
    z4 = -1 * (-4 * i + j + 2*N) / transition 
    ...
    if j < N//2:
      Du[i,j] = scale * tanh_bottom
    else: 
      Du[i,j] = scale * tanh_top #orginal
...
total_V_concentration = np.zeros(N)
